...
const networkStatusDesc = document.getElementById('network-status-desc');
...
// 🌟 FIREBASE PRESENCE & CONNECTION LOGIC 🌟
function initFirebasePresence() {
  db.ref('.info/connected').on('value', snap => {
    if (snap.val() === true) {
      ...
      networkStatusDesc.textContent = "You are connected! Online players:";
      ...
      networkStatusDesc.textContent = "Reconnecting...";
    }
  });
  ...
  db.ref('players').on('value', snap => {
    ...
    for(let id in players) {
      ...
      pRow.querySelector('button').addEventListener('click', () => {
         showToast(`Trading system coming soon!`);
      });
