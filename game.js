...
function getGlobalShopStockForCycle(cycleId, luckMult = 1.0) {
  ...
  SEED_CATALOG.forEach(s => {
    ...
    if (luckMult >= 100 && (s.id === 'singularity' || s.id === 'celestial_moon')) {
      adjustedChance = 1.0; // 100X guarantees transcendent
    }

    stockMap.seeds[s.id] = (s.rarity === 'common' || rng() < adjustedChance) ? s.maxStock : 0;
  });

  // Event Seeds (Holofern is exact 0.01% on its own, scaled by luck)
...
// ==========================================================================
// 🌧️ WEATHER SYNCHRONIZATION ENGINE & OVERRIDES
// ==========================================================================
...
function updateGlobalCycle() {
  ...
  if (gameState.weatherOverride) {
    // Admin Override forces Prismatic Paint Rain indefinitely
...
if (adminBtnTriggerLuck) {
  adminBtnTriggerLuck.addEventListener('click', () => {
    ...
    if (adminTargetScope.value === 'global' && !isPlaytesterMode) {
      db.ref('adminCommands').push({ type: 'luck', multiplier: luckMult, ts: getServerTime() });
      showToast(`Sent Global Luck Command: ${luckMult}X`);
    } else {
      gameState.restockLuckMultiplier = luckMult;
      gameState.lastShopCycle = null; // Force recalculation
...
function initFirebasePresence() {
  ...
  // Global Admin Commands Listener
  ...
  db.ref('adminCommands').limitToLast(20).on('child_added', snap => {
    ...
    if (cmd.ts && cmd.ts >= clientStartTime - 5000) {
      ...
      } else if (cmd.type === 'luck') {
        gameState.restockLuckMultiplier = cmd.multiplier;
        gameState.lastShopCycle = null;
        updateShopForCurrentCycle();
        showToast(`🍀 Admin applied ${cmd.multiplier}X Restock Luck for everyone!`);
      }
    }
  });
