...
function updateHUD() {
  ...
  if (adminWeatherTag) {
    adminWeatherTag.classList.toggle('hidden', !gameState.weatherOverride || gameState.weatherOverride === 'natural');
  }
...
function updateGlobalCycle() {
  ...
  if (gameState.weatherOverride === 'rain' || gameState.weatherOverride === true) {
    gameState.isDay = false;
    gameState.isPrismaticRain = true;
    gameState.cycleTimeLeft = 999;
  } else if (gameState.weatherOverride === 'clear') {
...
function initFirebasePresence() {
  ...
  db.ref('globalWeatherOverride').on('value', snap => {
    const val = snap.val();
    if (val) {
      if (typeof val.mode === 'string') {
        gameState.weatherOverride = val.mode;
      } else if (typeof val.active === 'boolean') {
        gameState.weatherOverride = val.active ? 'rain' : 'clear';
      }
    } else {
      gameState.weatherOverride = 'clear';
    }
    updateGlobalCycle();
    updateHUD();
  });
...
function loadGame() {
  ...
  if (sd) {
    try {
      ...
      gameState.weatherOverride = d.weatherOverride || 'clear';
...
function setupDOMEventListeners() {
  ...
  on('admin-btn-toggle-weather', 'click', () => {
    const willRain = !(gameState.weatherOverride === 'rain' || gameState.isPrismaticRain);
    const targetMode = willRain ? 'rain' : 'clear';
    ...
    gameState.weatherOverride = targetMode;
    updateGlobalCycle();
    updateHUD();
    showToast(willRain ? "🌧️ Prismatic Rain ENABLED for everyone!" : "☀️ Weather CLEARED for everyone!");
  });
