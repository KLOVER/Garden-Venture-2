...
// 🌟 ECONOMY REBALANCED: High tier sell prices slightly lowered to require actual farming
const SEED_CATALOG = [
...
let isTitanBargainerActive = false; // TITAN ADMIN VARIABLE
...
// 🌟 STOCK ODDS BUFFED: MUCH easier to find specific rare items 🌟
function refreshShopStocks(silent = false) {
  SEED_CATALOG.forEach(s => {
    ...
    if (s.rarity === 'common') {
      ...
      c = 0.50;
    } else if (s.rarity === 'legendary') {
      c = 0.10;
    }
    
    // SPECIFIC OVERRIDES REQUESTED
    if (s.id === 'sunflower') { c = 0.75; }
    else if (s.id === 'nectarroot') { c = 0.50; }
    else if (s.id === 'strawberry') { c = 0.15; }
    else if (s.id === 'cosmic_rose') { c = 0.08; }
    else if (s.id === 'singularity') { c = 0.03; }
    else if (s.id === 'celestial_moon') { c = 0.01; }
...
// 🌟 MATH FIX: Weight multiplier logarithmically scales to prevent $100M Carrots 🌟
function calculateProduceEarnings(base, rKg, minKg, isVine = false) {
  const sKg = rKg || minKg || 0.1;
  const weightRatio = sKg / (minKg || 0.1);
  let m = 1 + Math.pow(weightRatio, 0.55) * 0.15; // Logarithmic scaling
  if (isVine) {
    m *= 0.3;
  }
    ...
    if (plot.crop) {
      ...
      if (plot.crop.isVine) {
        if (!plot.vineEstablished) {
          ...
          n.cropTimerBadge.textContent = `🌱 ${formatMeters(cM)} | ${spB}${rS}s`;
        ...
        n.cropTimerBadge.textContent = `🌱 ${formatKg(cKg)} | ${spB}${rS}s`;
        n.cropTimerBadge.classList.remove('ready-badge');
        n.growthBar.style.display = 'block';
        n.progress.style.width = `${Math.min(100, plot.progress)}%`;
      } else {
...
// 🌟 FIX: Seed Bag Tabs Event Listeners added directly for controller/click interactions
...
// 🌟 FIX: MANUAL RESET FOR SANDBOXED ENVIRONMENTS 🌟
btnConfirmReset.addEventListener('click', () => {
  localStorage.removeItem('gardenVenture2Save');
  
  // Hard reset local state immediately instead of waiting for reload
  gameState = {
    cash: 25,
    level: 1,
    xp: 0,
  ...
  // Attempt reload as a backup if environment allows it
...
adminBtnCash.addEventListener('click', () => {
  ...
  if (adminTargetScope.value === 'global') {
    db.ref('adminCommands').push({ type: 'cash', amount: q, ts: Date.now() });
...
adminBtnGrant.addEventListener('click', () => {
  ...
  if (adminTargetScope.value === 'global') {
    db.ref('adminCommands').push({ type: 'seed', seedId: id, amount: q, ts: Date.now() });
    showToast(`Sent Global Command: +${q} ${id}`);
  } else {
    gameState.seedInventory[id] += q;
    updateHUD();
...
// 🌟 NEW ADMIN COMMAND: SKIP GROWING 🌟
...
// 🌟 NEW ADMIN COMMAND: TITAN BARGAINER 🌟
...
adminBtnBroadcast.addEventListener('click', () => {
  ...
  if (m.trim() !== '') {
    db.ref('adminCommands').push({ type: 'broadcast', message: m, ts: Date.now() });
...
btnSendChat.addEventListener('click', () => {
  ...
  if (msg !== '' && isOnline) {
    db.ref('chat').push({ sender: myPlayerId, text: msg, timestamp: Date.now() });
    chatInput.value = '';
  }
});
...
function initFirebasePresence() {
  ...
  db.ref('adminCommands').limitToLast(1).on('child_added', snap => {
    const cmd = snap.val();
    if (cmd && cmd.ts > initTime) {
      if (cmd.type === 'cash') {
...
// 🌟 GLOBAL SYNCHRONIZED DAY/NIGHT CYCLE 🌟
function updateGlobalCycle() {
  const CYCLE_DURATION = 600000; // 10 minutes (600,000 ms)
  const HALF_CYCLE = 300000;     // 5 minutes day / 5 minutes night
  const nowOffset = Date.now() % CYCLE_DURATION;
...
function secondTick() {
  updateGlobalCycle(); // 🌟 Calls the synchronized cycle logic
  
  let sLeft = Math.floor((globalShopExpiresAt - Date.now()) / 1000);
  if (sLeft <= 0) {
    sLeft = 0;
    if (isOnline) {
      db.ref('globalShop').once('value', s => {
        if (s.val() && s.val().expiresAt < Date.now()) {
          db.ref('globalShop').remove();
        }
      });
    }
  }
...
// ==========================================
// SYSTEM & NETWORK HELPER FUNCTIONS
// ==========================================
...
function initGlobalShop() {
  ...
  db.ref('globalShop').on('value', snap => {
    ...
    if (data && data.expiresAt && data.expiresAt > Date.now()) {
      ...
      globalShopExpiresAt = Date.now() + 180000;
      if (isOnline) {
        db.ref('globalShop').set({ expiresAt: globalShopExpiresAt });
      }
      refreshShopStocks(true);
    }
  });
...
// 🌟 ADOPT ME STYLE 3x3 TRADE GRID SYSTEM 🌟
...
function initGame() {
  ...
  // Enforce immediate global day/night alignment on load
  updateGlobalCycle();
