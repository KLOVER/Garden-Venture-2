...
let currentSkipTarget = null, lastSkipTime = 0; 
...
function refreshShopStocks(silent = false) {
  SEED_CATALOG.forEach(seed => {
    ...
    seed.currentStock = Math.random() < chance ? seed.maxStock : 0;
  });
...
function addXP(amount) {
  ...
  if (leveledUp) {
    ...
    // UPDATE SERVER PRESENCE ON LEVEL UP
...
// TRADE TABS
const tabPublicTrade = document.getElementById('tab-public-trade');
const tabPrivateTrade = document.getElementById('tab-private-trade');
const publicTradeView = document.getElementById('public-trade-view');
const privateTradeView = document.getElementById('private-trade-view');

const privateTradeIdInput = document.getElementById('private-trade-id-input');
const btnSendPrivateTrade = document.getElementById('btn-send-private-trade');

// TRADE REQUEST MODAL
const tradeRequestModal = document.getElementById('trade-request-modal');
const tradeRequestId = document.getElementById('trade-request-id');
...
// TRADE SESSION SPLIT VIEW
const tradeSessionModal = document.getElementById('trade-session-modal');
const tradeStatusIndicator = document.getElementById('trade-status-indicator');
const tradePartnerTitle = document.getElementById('trade-partner-title');
const myTradeSlots = document.getElementById('my-trade-slots');
const theirTradeSlots = document.getElementById('their-trade-slots');
const theirTradeStatus = document.getElementById('their-trade-status');
...
// TRADE BACKPACK
const tradeBackpackModal = document.getElementById('trade-backpack-modal');
const tradeTabSeeds = document.getElementById('trade-tab-seeds');
const tradeTabProduce = document.getElementById('trade-tab-produce');
const tradePickerList = document.getElementById('trade-picker-list');
const btnCloseTradeBackpack = document.getElementById('btn-close-trade-backpack');
...
tabPublicTrade.addEventListener('click', () => {
  tabPublicTrade.classList.add('active'); tabPrivateTrade.classList.remove('active');
  publicTradeView.classList.remove('hidden'); privateTradeView.classList.add('hidden');
});

tabPrivateTrade.addEventListener('click', () => {
  tabPrivateTrade.classList.add('active'); tabPublicTrade.classList.remove('active');
  privateTradeView.classList.remove('hidden'); publicTradeView.classList.add('hidden');
});

// EVENT LISTENERS: TRADE ACTION BUTTONS
function sendTradeRequest(targetId) {
  ...
  db.ref('players/' + targetId).once('value', snap => {
    if(snap.exists() && snap.val().online) {
       db.ref('players/' + targetId + '/tradeRequest').set({ from: myPlayerId });
       showToast("📩 Request sent to " + targetId);
    } else {
       showToast("❌ Player not found or offline.");
    }
  });
}

btnSendPrivateTrade.addEventListener('click', () => {
  const targetId = privateTradeIdInput.value.trim();
  if(targetId && targetId !== myPlayerId) { sendTradeRequest(targetId); }
});
...
btnAcceptTrade.addEventListener('click', () => {
  const partner = gameState.pendingTradeReq;
  const tradeId = myPlayerId + "_" + partner + "_" + Date.now();
  
  db.ref('trades/' + tradeId).set({ p1: myPlayerId, p2: partner, status: 'negotiating' });
  db.ref('players/' + myPlayerId + '/tradeRequest').remove();
  db.ref('players/' + myPlayerId + '/activeTrade').set(tradeId);
  db.ref('players/' + partner + '/activeTrade').set(tradeId);
});

btnTradeCancel.addEventListener('click', () => {
   if(gameState.currentTradeId) {
      db.ref('trades/' + gameState.currentTradeId).remove();
      db.ref('players/' + myPlayerId + '/activeTrade').remove();
   }
});
...
btnCloseTradeBackpack.addEventListener('click', () => {
  closeModal(tradeBackpackModal);
});

function renderTradeBackpack() {
  tradePickerList.innerHTML = `<p style="text-align: center; color: #795548; font-weight: 800; padding: 20px;">Select an item to add to your offer...</p>`;
}
...
function initFirebasePresence() {
  ...
  db.ref('.info/connected').on('value', snap => {
    if (snap.val() === true) {
      ...
      myRef.set({ level: gameState.level, online: true });
    } else {
  ...
  db.ref('players').on('value', snap => {
    ...
    for(let id in players) {
      ...
      pRow.innerHTML = `<span style="font-weight: 900; color: #4e342e;">ID: ${id} <span style="font-size: 12px; color: #f57c00;">(Lvl ${pData.level})</span></span> <button class="btn" style="padding: 6px 12px; font-size: 12px; background: #8e24aa; box-shadow: 0 3px 0 #6a1b9a;">Trade</button>`;
      
      pRow.querySelector('button').addEventListener('click', () => {
         sendTradeRequest(id);
      });
    ...
    if(count === 0) {
       activePlayersList.innerHTML = `<p style="text-align: center; color: #795548; font-weight: 800; font-size: 14px;">No other players online right now.</p>`;
    }
  });
  ...
  db.ref('players/' + myPlayerId + '/tradeRequest').on('value', snap => {
    ...
    if (req && req.from) {
       ...
       gameState.pendingTradeReq = req.from;
    } else {
       closeModal(tradeRequestModal);
       gameState.pendingTradeReq = null;
    }
  });

  // Listen for ACTIVE Trade Sessions
  db.ref('players/' + myPlayerId + '/activeTrade').on('value', snap => {
     const tradeId = snap.val();
     if(tradeId) {
        gameState.currentTradeId = tradeId;
        closeModal(friendsModal);
        closeModal(tradeRequestModal);
        
        // Setup visual split screen
        myTradeSlots.innerHTML = ''; theirTradeSlots.innerHTML = '';
        for(let i=0; i<9; i++) {
           myTradeSlots.innerHTML += `<div class="trade-slot"></div>`;
           theirTradeSlots.innerHTML += `<div class="trade-slot"></div>`;
        }
        
        openModal(tradeSessionModal);
     } else {
        closeModal(tradeSessionModal);
        gameState.currentTradeId = null;
        if(document.getElementById('trade-session-modal').classList.contains('open')) {
...
function openWeatherModal() {
  ...
  if (gameState.isDay) {
    weatherModalBody.innerHTML = `
      ...
      <p style="margin-top: 10px;"><strong>☀️ Solar Boost (2X Growth Speed):</strong><br>🌻 Sunflower</p>
    `;
  } else {
    weatherModalBody.innerHTML = `
