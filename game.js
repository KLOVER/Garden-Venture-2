...
function generateShopStockMap() {
  ...
  SEED_CATALOG.forEach(s => {
    let c = 0;
    if (s.rarity === 'common') c = 1.0;
    else if (s.rarity === 'uncommon') c = 0.75;
    ...
    stockMap[s.id] = (Math.random() < c) ? s.maxStock : 0;
  });
  return stockMap;
}
...
// 🌟 FIREBASE STOCK SYNC: Saves actual seed stock to Firebase for Tracker Apps / Bots 🌟
...
function initGlobalShop() {
  ...
  db.ref('globalShop').on('value', snap => {
    const data = snap.val();
    if (data && data.expiresAt && data.expiresAt > getServerTime()) {
