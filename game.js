...
// 🪵 12 FENCE SKINS (BALANCED STOCK RATIOS)
...
// 🛒 CALCULATE CYCLE STOCKS WITH PRECISE RARITY MATHEMATICS
function getGlobalShopStockForCycle(cycleId) {
  ...
  SEED_CATALOG.forEach(s => {
    ...
    stockMap.seeds[s.id] = (s.rarity === 'common' || rng() < c) ? s.maxStock : 0;
  });

  // Event Seeds (Holofern is exact 0.01% on its own)
  EVENT_SEED_CATALOG.forEach(s => {
    let c = 0.60;
    if (s.id === 'splatterbloom') c = 0.10;
    else if (s.id === 'holofern') c = 0.0001; // Exact 0.01% chance

    stockMap.eventSeeds[s.id] = (rng() < c) ? s.maxStock : 0;
  });

  // Fence Skins (Transcendent is exact 0.1% chance)
  FENCE_SKINS_CATALOG.forEach(skin => {
    let c = 1.0;
    if (skin.rarity === 'uncommon') c = 0.60;
    else if (skin.rarity === 'rare') c = 0.25;
    else if (skin.rarity === 'legendary') c = 0.05;
    else if (skin.rarity === 'astral') c = 0.01;
    else if (skin.rarity === 'transcendent') c = 0.001; // Exact 0.1% chance

    stockMap.fences[skin.id] = (skin.rarity === 'common' || rng() < c) ? skin.maxStock : 0;
  });
...
function checkHolographicRainMutations() {
  ...
  currentFieldPlots.forEach((plot, i) => {
    if (plot.crop && !plot.crop.isEventSeed && !plot.isHoloMutated) {
      if (Math.random() < 0.05) { // Balanced rare mutation chance during rain
...
function renderPlots() {
  ...
  cP.forEach((plot, i) => {
    ...
    if (plot.crop) {
      ...
      const maxS = Math.min(2.5, 0.8 + Math.log10(tKg + 1) * 0.35); // Clamped scaling to prevent overflow
