...
const SEED_CATALOG = [
  ...
  // 🔴 SPECIAL ADMIN ROTATION SEEDS
  { id: 'admin_cortex', name: 'ADMIN CORTEX', icon: '🌪️', rarity: 'admin', affinity: 'all', coinCost: 500, maxStock: 1, currentStock: 1, baseGrowTime: 180, baseSellPrice: 250000000, minKg: 10.0, baseMaxKg: 50.0, maxKg: 1000000000, minM: 5.0, maxM: 25.0, isVine: false, cssClass: 'glitch-cortex', isAdminSeed: true },
  { id: 'casden_seed', name: 'The Casden Seed', icon: '🧍', rarity: 'admin', affinity: 'all', coinCost: 750, maxStock: 1, currentStock: 1, baseGrowTime: 240, baseSellPrice: 500000000, minKg: 15.0, baseMaxKg: 60.0, maxKg: 2000000000, minM: 10.0, maxM: 30.0, isVine: false, noSproutStage: true, isAdminSeed: true },
  { id: 'admins_last_wish', name: 'ADMINS LAST WISH', icon: '🌷', rarity: 'admin', affinity: 'all', coinCost: 1500, maxStock: 1, currentStock: 1, baseGrowTime: 300, baseSellPrice: 1000000000, minKg: 20.0, baseMaxKg: 100.0, maxKg: 1000000000000, minM: 15.0, maxM: 100.0, isVine: false, cssClass: 'glitch-wish', isAdminSeed: true }
];
...
let gameState = {
  ...
  isAdminRotationActive: false,
  stockLuckMultiplier: 1,
  ...
  sfxMuted: false,
  dailyDealUsed: false,
  seedInventory: { carrot: 5, potato: 0, grape_vine: 0, tomato: 0, glowshroom: 0, starfruit: 0, watermelon_vine: 0, sunflower: 0, nectarroot: 0, strawberry: 0, cosmic_rose: 0, singularity: 0, celestial_moon: 0, admin_cortex: 0, casden_seed: 0, admins_last_wish: 0 },
  produceInventory: [],
...
function getGlobalShopStockForCycle(cycleId) {
  ...
  const stockMap = {};
  const luck = gameState.stockLuckMultiplier || 1;

  SEED_CATALOG.forEach(s => {
    if (s.isAdminSeed) {
      stockMap[s.id] = gameState.isAdminRotationActive ? 1 : 0;
      return;
    }
    let c = 1.0;
    ...
    const effectiveChance = Math.min(1.0, c * luck);
    const effectiveStock = Math.min(999, Math.ceil(s.maxStock * Math.min(luck, 5)));

    stockMap[s.id] = (s.rarity === 'common' || rng() < effectiveChance) ? effectiveStock : 0;
  });
...
const adminCoinHudEl = el('admin-coin-hud');
const adminCoinAmountEl = el('admin-coin-amount');
...
const shopTabAdmin = el('shop-tab-admin');
const adminCoinBuyBox = el('admin-coin-buy-box');
const btnBuyAdminCoin = el('btn-buy-admin-coin');
...
const adminStockLuck = el('admin-stock-luck');
const adminBtnRotation = el('admin-btn-rotation');
const adminCashQty = el('admin-cash-qty');
const adminBtnCash = el('admin-btn-cash');
const adminCoinQty = el('admin-coin-qty');
const adminBtnCoins = el('admin-btn-coins');
...
function formatKg(kg) {
  ...
  const v = Number(kg);
  if (v >= 1e12) return (v / 1e12).toFixed(2) + 'T kg';
...
function calculateProduceEarnings(base, rKg, minKg, isVine = false, isAdminSkipped = false) {
  ...
  if (isAdminSkipped) {
    total = Math.max(1, Math.round(total * 0.5)); // ⚖️ 50% Admin Skip Penalty applied
  }
  return total;
}
...
function updateHUD() {
  ...
  if (adminCoinHudEl && adminCoinAmountEl) {
    if (gameState.isAdminRotationActive || gameState.adminCoins > 0) {
      adminCoinHudEl.classList.remove('hidden');
      adminCoinAmountEl.textContent = gameState.adminCoins.toLocaleString();
    } else {
      adminCoinHudEl.classList.add('hidden');
    }
  }
...
function renderShopItems() {
  ...
  const isAdminTab = (gameState.activeShopTab === 'admin');
  
  if (shopTabAdmin) {
    shopTabAdmin.classList.toggle('hidden', !gameState.isAdminRotationActive);
  }
  if (adminCoinBuyBox) {
    adminCoinBuyBox.classList.toggle('hidden', !isAdminTab);
  }

  SEED_CATALOG.forEach(s => {
    if (s.isAdminSeed && !isAdminTab) return;
    if (!s.isAdminSeed && isAdminTab) return;

    const isCoinSeed = !!s.coinCost;
    const aff = isCoinSeed ? (gameState.adminCoins >= s.coinCost) : (gameState.cash >= s.cost);
    const stk = s.currentStock > 0;
    const own = gameState.seedInventory[s.id] || 0;
    
    const c = document.createElement('div');
    c.className = `shop-item-card ${s.isAdminSeed ? 'admin-shop-card' : ''}`;
    
    let btnClass = 'btn-buy';
    let btnText = isCoinSeed ? `Buy (${s.coinCost} 🪙)` : 'Buy';
    ...
    const priceText = isCoinSeed ? `${s.coinCost} Admin Coins 🪙` : `${formatCash(s.cost)}`;
    c.innerHTML = `
      <div class="item-info">
        <div class="item-title">${s.icon} ${s.name} ${badgeHtml}</div>
        <div><span class="rarity-tag rarity-${s.rarity}">${s.rarity}</span></div>
        <div class="item-price-stock">${priceText} | Stock: ${s.currentStock} (Owned: ${own})</div>
      </div>
    ...
    if (aff && stk) {
      c.querySelector('.btn-buy').addEventListener('click', () => {
        if (isCoinSeed) {
          gameState.adminCoins -= s.coinCost;
        } else {
...
function renderSeedDrawer() {
  ...
  if (gameState.activeDrawerTab === 'seeds') {
    ...
    gameState.produceInventory.forEach(i => {
      ...
      const penaltyTag = i.isAdminSkipped ? ' <span style="color:#d32f2f; font-size:10px;">(50% Skip Penalty)</span>' : '';
      c.innerHTML = `<div style="display:flex; align-items:center; gap:10px; width:100%;"><div style="font-size:26px;">${i.icon}</div><div style="display:flex; flex-direction:column; flex:1;"><span style="font-size:13px; font-weight:900; color:#2c1a14;">${i.name}${penaltyTag}</span><span style="font-size:11px; color:#2e7d32; font-weight:800;">Weight: ${formatKg(i.kg)} | Value: ${formatCash(i.value)}</span></div></div>`;
...
function renderSellMainOptions() {
  ...
  sellAllPayoutText.textContent = `Total Value: ${formatCash(t)} (${gameState.produceInventory.length} items)`;
}
...
function renderSellItemPicker() {
  ...
  getGroupedProduce().forEach(g => {
    ...
    c.innerHTML = `<span style="font-size: 28px;">${g.icon}</span><div class="sell-opt-text" style="flex: 1;"><span class="opt-title">${g.name} (x${g.items.length})</span><span class="opt-subtitle">Total Value: ${formatCash(tv)}</span></div><button class="btn-market-select">${g.items.length > 1 ? 'Choose Qty 🧺' : `Sell 1 (${formatCash(tv)})`}</button>`;
    ...
    c.addEventListener('click', () => {
      if (g.items.length === 1) {
        ...
        playSFX('sell');
        createFloatingText(window.innerWidth / 2, window.innerHeight / 2, `+${formatCash(i.value)}! 💰`, "#ffd54f");
        updateHUD();
        saveGame();
...
function renderPlots() {
  ...
  cP.forEach((plot, i) => {
    ...
    if (plot.crop) {
      ...
      // Casden seed grows directly without sprout
      if (plot.crop.noSproutStage) {
        n.cropIcon.textContent = plot.crop.icon;
      } else {
        n.cropIcon.textContent = plot.progress < 35 ? '🌱' : plot.crop.icon;
      }

      n.cropIcon.className = `crop-icon ${plot.crop.cssClass || ''} ${plot.isReady || plot.vineEstablished ? 'mature' : ''}`;
      n.cropTimerBadge.style.display = 'flex';
      
      let spB = '';
      if (plot.crop.affinity === 'night' && !gameState.isDay) spB = '⚡2X ';
      if (plot.crop.affinity === 'day' && gameState.isDay) spB = '☀️2X ';
      if (plot.isAdminSkipped) spB += '⚠️50% ';
...
function handlePlotClick(pI, e) {
  ...
  if (p.crop && p.isReady) {
    ...
    const earn = calculateProduceEarnings(p.crop.baseSellPrice, sKg, p.crop.minKg, false, p.isAdminSkipped);
    addXP(Math.ceil(p.crop.baseGrowTime * 1.5));
    gameState.produceInventory.push({ 
      id: Date.now() + Math.random(), 
      seedId: p.crop.id, 
      name: p.crop.name.replace(' Seed', ''), 
      icon: p.crop.icon, 
      kg: sKg, 
      meters: p.rolledMeters || p.crop.minM || 1, 
      value: earn,
      isAdminSkipped: p.isAdminSkipped 
    });
    ...
    if (currentSkipTarget.isFruit) {
      ...
      if (p.crop.isVine) {
        ...
        for (let f = 0; f < Math.min(3, p.crop.maxFruits || 3); f++) {
          ...
          p.vineFruits.push({ fruitId: `${p.crop.id}_${f}`, name: p.crop.produceName, icon: p.crop.produceIcon, progress: 0, isReady: false, rolledKg: r.fruitKg, growTime: r.fruitGrowTime, isAdminSkipped: false });
        }
      } else {
        p.isReady = true;
      }
...
function renderVineModalContent() {
  ...
  (p.vineFruits || []).forEach((f, i) => {
    ...
    const earn = calculateProduceEarnings(p.crop.baseSellPrice, sKg, p.crop.minKg, true, f.isAdminSkipped);
    ...
    cd.innerHTML = `<div class="card-left-group"><div class="item-icon-badge">${f.icon}</div><div class="item-details"><span class="item-title">${f.name} #${i + 1}</span><span class="item-sub-stat">${f.isReady ? `${formatKg(sKg)} • <strong style="color:#2e7d32;">${formatCash(earn)}</strong>` : `🌱 ${formatKg((f.progress / 100) * sKg)}`}</span></div></div>${f.isReady ? `<button class="btn-vine-harvest">Harvest</button>` : `<button class="btn-vine-harvest" style="background:#29b6f6; box-shadow:0 4px 0 #0288d1;">Skip (⏳ ${formatTime(rS)})</button>`}`;
    if (f.isReady) {
      cd.querySelector('.btn-vine-harvest').addEventListener('click', () => {
        addXP(Math.ceil((p.crop.baseGrowTime * 1.5) / (p.crop.maxFruits || 3)));
        gameState.produceInventory.push({ id: Date.now() + Math.random(), seedId: p.crop.id, name: f.name, icon: f.icon, kg: sKg, meters: p.crop.minM, value: earn, isAdminSkipped: f.isAdminSkipped });
        f.progress = 0;
        f.isReady = false;
        f.isAdminSkipped = false;
...
harvestAllVineBtn.addEventListener('click', () => {
  ...
  (p.vineFruits || []).forEach(f => {
    if (f.isReady) {
      const sKg = f.rolledKg || p.crop.minKg || 0.1;
      gameState.produceInventory.push({ id: Date.now() + Math.random(), seedId: p.crop.id, name: f.name, icon: f.icon, kg: sKg, meters: p.crop.minM, value: calculateProduceEarnings(p.crop.baseSellPrice, sKg, p.crop.minKg, true, f.isAdminSkipped), isAdminSkipped: f.isAdminSkipped });
      addXP(Math.ceil((p.crop.baseGrowTime * 1.5) / (p.crop.maxFruits || 3)));
      f.progress = 0;
      f.isReady = false;
      f.isAdminSkipped = false;
...
if (shopTabNormal) {
  shopTabNormal.addEventListener('click', () => {
    ...
    if (shopTabAdmin) shopTabAdmin.classList.remove('active');
    renderShopItems();
  });
}

if (shopTabAdmin) {
  shopTabAdmin.addEventListener('click', () => {
    gameState.activeShopTab = 'admin';
    shopTabAdmin.classList.add('active');
...
if (btnBuyAdminCoin) {
  btnBuyAdminCoin.addEventListener('click', () => {
    if (gameState.cash >= 1000000) {
      gameState.cash -= 1000000;
      gameState.adminCoins += 1;
      playSFX('sell');
      showToast('🪙 Converted $1,000,000 Cash -> 1 Admin Coin!');
      updateHUD();
      renderShopItems();
      saveGame();
    } else {
      showToast('❌ Need $1,000,000 Cash to convert for 1 Admin Coin!');
    }
  });
}
...
btnSellAllModal.addEventListener('click', () => {
  ...
  gameState.cash += t;
  playSFX('sell');
  showToast(`💰 Sold All for ${formatCash(t)}!`);
  updateHUD();
  saveGame();
  closeModal(sellModal);
});
...
btnSellBargainModal.addEventListener('click', () => {
  ...
  gameState.produceInventory.forEach(i => {
    bv += i.value;
    if (i.seedId === 'singularity' || i.seedId === 'celestial_moon' || i.seedId === 'strawberry' || i.seedId === 'cosmic_rose' || i.seedId === 'admin_cortex' || i.seedId === 'casden_seed' || i.seedId === 'admins_last_wish') hR++;
  });
  ...
  npcDialogueText.textContent = isTitanBargainerActive 
      ? `TITAN MODE ACTIVE! My wealth is limitless! Appraisal fee: ${formatCash(currentBargainFee)}.` 
      : `Greetings! I can appraise your Seed Bag. My fee is ${formatCash(currentBargainFee)}.`;
...
qtyConfirmSellBtn.addEventListener('click', () => {
  ...
  gameState.cash += t;
  playSFX('sell');
  showToast(`💰 Sold ${c}x for ${formatCash(t)}!`);
  updateHUD();
  saveGame();
  if (gameState.produceInventory.length === 0) {
    closeModal(sellModal);
...
if (adminStockLuck) {
  adminStockLuck.addEventListener('change', () => {
    const mult = parseInt(adminStockLuck.value) || 1;
    if (adminTargetScope.value === 'global') {
      db.ref('adminCommands').push({ type: 'stockLuck', multiplier: mult, ts: getServerTime() });
      showToast(`🎲 Broadcasted Global Stock Luck Multiplier: ${mult}X!`);
    } else {
      gameState.stockLuckMultiplier = mult;
      updateShopForCurrentCycle(true);
      showToast(`🎲 Local Stock Luck Multiplier set to ${mult}X!`);
    }
  });
}

if (adminBtnRotation) {
  adminBtnRotation.addEventListener('click', () => {
    const nextState = !gameState.isAdminRotationActive;
    if (adminTargetScope.value === 'global') {
      db.ref('adminCommands').push({ type: 'adminRotation', active: nextState, ts: getServerTime() });
      showToast(`🌪️ Broadcasted Admin Rotation: ${nextState ? 'ON' : 'OFF'}`);
    } else {
      gameState.isAdminRotationActive = nextState;
      adminBtnRotation.textContent = `Toggle Admin Rotation (${nextState ? 'ON' : 'OFF'})`;
      updateHUD();
      renderShopItems();
      showToast(`🌪️ Admin Rotation set to ${nextState ? 'ON' : 'OFF'}`);
    }
  });
}
...
if (adminBtnCoins) {
  adminBtnCoins.addEventListener('click', () => {
    const q = parseInt(adminCoinQty.value) || 0;
    if (q <= 0) return;
    if (adminTargetScope.value === 'global') {
      db.ref('adminCommands').push({ type: 'coins', amount: q, ts: getServerTime() });
      showToast(`Sent Global Command: +${q} Admin Coins!`);
    } else {
      gameState.adminCoins += q;
      playSFX('sell');
      updateHUD();
      showToast(`🛡️ Granted ${q} Admin Coins 🪙!`);
    }
  });
}
...
function skipAllPlayerCrops() {
  let skipped = 0;
  gameState.fields.forEach((field) => {
    field.forEach((p) => {
      if (p.crop) {
        p.isAdminSkipped = true; // ⚖️ Flag crop for 50% sell value penalty
        if (p.crop.isVine) {
          if (!p.vineEstablished) {
            ...
            for (let f = 0; f < Math.min(3, p.crop.maxFruits || 3); f++) {
              const r = rollFruitStats(p.crop);
              p.vineFruits.push({ fruitId: `${p.crop.id}_${f}`, name: p.crop.produceName, icon: p.crop.produceIcon, progress: 100, isReady: true, rolledKg: r.fruitKg, growTime: r.fruitGrowTime, isAdminSkipped: true });
            }
            skipped++;
  ...
  if (skipped > 0) {
    playSFX('sell');
    createFloatingText(window.innerWidth / 2, window.innerHeight / 2, "⏩ FAST FORWARDED! (50% Value Penalty)", "#00bcd4");
    updateHUD();
    renderPlots();
  }
  return skipped;
...
adminBtnSkipGrow.addEventListener('click', () => {
  if (adminTargetScope.value === 'global') {
    db.ref('adminCommands').push({ type: 'skipGrow', ts: getServerTime() });
    showToast(`🚀 Sent Global Command: Fast-Forward All Crops!`);
  } else {
    const skipped = skipAllPlayerCrops();
    if (skipped > 0) {
      showToast("🚀 Fast-forwarded all crops on farm! (50% Value Penalty Applied)");
    } else {
...
function initFirebasePresence() {
  ...
  db.ref('adminCommands').limitToLast(20).on('child_added', snap => {
    ...
    if (cmd.ts && cmd.ts >= clientStartTime - 5000) {
      if (cmd.type === 'cash') {
      ...
      } else if (cmd.type === 'coins') {
        gameState.adminCoins += cmd.amount;
        updateHUD();
        playSFX('sell');
        showToast(`👑 Admin granted everyone ${cmd.amount} Admin Coins 🪙!`);
        ...
        skipAllPlayerCrops();
        showToast(`⚡ Admin fast-forwarded crop growth for everyone! (50% Value Penalty Applied)`);
      } else if (cmd.type === 'stockLuck') {
        gameState.stockLuckMultiplier = cmd.multiplier;
        updateShopForCurrentCycle(true);
        showToast(`🎲 Stock Luck Multiplier set to ${cmd.multiplier}X!`);
      } else if (cmd.type === 'adminRotation') {
        gameState.isAdminRotationActive = cmd.active;
        if (adminBtnRotation) adminBtnRotation.textContent = `Toggle Admin Rotation (${cmd.active ? 'ON' : 'OFF'})`;
        updateHUD();
        renderShopItems();
        showToast(`🌪️ Admin Rotation set to ${cmd.active ? 'ON' : 'OFF'}`);
          ...
          if (!p.vineEstablished) {
            ...
            if (p.progress >= 100) {
              ...
              for (let i = 0; i < Math.min(3, p.crop.maxFruits || 3); i++) {
                const rs = rollFruitStats(p.crop);
                p.vineFruits.push({ fruitId: `${p.crop.id}_${i}`, name: p.crop.produceName, icon: p.crop.produceIcon, progress: 0, isReady: false, rolledKg: rs.fruitKg, growTime: rs.fruitGrowTime, isAdminSkipped: p.isAdminSkipped });
              }
            }
          } else if (p.vineFruits) {
            p.vineFruits.forEach(fr => {
...
function saveGame() {
  localStorage.setItem('gardenVenture2Save', JSON.stringify({
    cash: gameState.cash,
    adminCoins: gameState.adminCoins,
    ...
    isAdminRotationActive: gameState.isAdminRotationActive,
    stockLuckMultiplier: gameState.stockLuckMultiplier,
...
function loadGame() {
  ...
  if (sd) {
    try {
      ...
      gameState.adminCoins = d.adminCoins || 0;
      ...
      gameState.isAdminRotationActive = d.isAdminRotationActive || false;
      gameState.stockLuckMultiplier = d.stockLuckMultiplier || 1;
      ...
      if (d.fields && Array.isArray(d.fields)) {
        ...
        while (gameState.fields.length < gameState.maxFields) {
          const p = [];
          for (let i = 0; i < 9; i++) {
            p.push({ crop: null, progress: 0, isReady: false, vineEstablished: false, rolledKg: 0, rolledMeters: 0, actualGrowTime: 5, vineFruits: [], isAdminSkipped: false });
          }
          gameState.fields.push(p);
        }
      }
                  ...
                  if (!p.vineEstablished) {
                    ...
                    if (p.progress >= 100) {
                      ...
                      for (let i = 0; i < Math.min(fCount, fCount); i++) {
                        ...
                        p.vineFruits.push({ fruitId: `${p.crop.id}_${i}`, name: fName, icon: fIcon, progress: 0, isReady: false, rolledKg: rs.fruitKg, growTime: rs.fruitGrowTime, isAdminSkipped: p.isAdminSkipped });
                      }
                      if (extraTime > 0) {
                        p.vineFruits.forEach(fr => {
...
function renderTradeBackpack(tab = 'seeds') {
  ...
  if (tab === 'seeds') {
    ...
    gameState.produceInventory.forEach((item) => {
      ...
      card.className = 'trade-item-card';
      card.style = "display:flex; justify-content:space-between; align-items:center; background:#fff; padding:8px; border-radius:8px; margin-bottom:6px; border:2px solid #d7ccc8;";
      card.innerHTML = `<span>${item.icon} ${item.name} (${formatCash(item.value)})</span> <button style="background:#4caf50; color:#fff; border:none; padding:4px 10px; border-radius:6px; font-weight:bold; cursor:pointer;">Add</button>`;
      card.querySelector('button').onclick = () => {
        if (amIReady) return;
        if (myOfferedItems.length >= 9) {
          showToast("❌ Trade box is full (Max 9)!");
        ...
        myOfferedItems.push({ type: 'produce', id: item.id, seedId: item.seedId, name: item.name, icon: item.icon, kg: item.kg, value: item.value, isAdminSkipped: item.isAdminSkipped });
        const isP1 = currentTradeId && currentTradeId.startsWith(myPlayerId + '_');
        db.ref('trades/' + currentTradeId + (isP1 ? '/p1Items' : '/p2Items')).set(myOfferedItems);
        closeModal(tradeBackpackModal);
      };
