...
function setupDOMEventListeners() {
  ...
  on('btn-verify-gv1', 'click', () => {
    ...
    if (code === 'GV1-VETERAN-BLOOM' || code === 'VENTURE-LEGACY') {
      gameState.isGv1Veteran = true;
      gameState.seedInventory['venturebloom'] = (gameState.seedInventory['venturebloom'] || 0) + 3;
      localStorage.setItem('gv1_veteran_key', 'true');
      showToast("🏆 GV1 Veteran Verified! +3 VentureBloom Seeds granted!");
      playSFX('mutate');
      saveGame();
      updateHUD();
    } else {
  ...
  on('admin-btn-trigger-luck', 'click', () => {
    ...
    if (scope === 'global' && db) {
      ...
      showToast(`🌐 Global Luck set to ${lVal}X!`);
    } else {
  ...
  on('admin-btn-toggle-weather', 'click', () => {
    ...
    if (scope === 'global' && db) {
      db.ref('globalWeatherOverride').set({ active: newState, timestamp: getServerTime() });
    } else {
      gameState.weatherOverride = newState;
      updateGlobalCycle();
      updateHUD();
    }
    showToast(`🌧️ Prismatic Override: ${newState ? 'ENABLED' : 'DISABLED'}`);
  });
  ...
  on('btn-confirm-reset', 'click', () => {
    ...
    if (rainLayer) rainLayer.classList.add('hidden');
