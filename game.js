...
let gamepadState = { connected: false, focusZone: 'plots', selectedPlotIndex: 4, hudButtonIndex: 0, navButtonIndex: 0, bottomButtonIndex: 0, menuFocusIndex: 0, lastButtonTime: 0, buttonCooldown: 170 };
...
function renderPlots() {
  ...
  document.querySelectorAll('.gamepad-focused').forEach(el => {
    if (!el.classList.contains('plot') || gamepadState.focusZone !== 'plots') el.classList.remove('gamepad-focused');
  });
...
function pollGamepad() {
  ...
  if (Math.abs(axisX) > 0.4 || Math.abs(axisY) > 0.4) {
    if (gamepadState.focusZone === 'plots') {
      ...
      } else if (axisX < -0.4 && col > 0) {
        col--; gamepadState.lastButtonTime = now;
      } else if (axisX > 0.4 && col < 2) {
        col++; gamepadState.lastButtonTime = now;
      }
      gamepadState.selectedPlotIndex = row * 3 + col;

    } else if (gamepadState.focusZone === 'nav') {
      if (axisY > 0.4) { gamepadState.focusZone = 'plots'; gamepadState.selectedPlotIndex = gamepadState.navButtonIndex === 0 ? 0 : 2; gamepadState.lastButtonTime = now; }
      else if (axisY < -0.4) { gamepadState.focusZone = 'hud'; gamepadState.hudButtonIndex = gamepadState.navButtonIndex === 0 ? 0 : 2; gamepadState.lastButtonTime = now; }
      else if (axisX < -0.4 && gamepadState.navButtonIndex > 0) { gamepadState.navButtonIndex--; gamepadState.lastButtonTime = now; }
      else if (axisX > 0.4 && gamepadState.navButtonIndex < 1) { gamepadState.navButtonIndex++; gamepadState.lastButtonTime = now; }

    } else if (gamepadState.focusZone === 'hud') {
      if (axisY > 0.4) { gamepadState.focusZone = 'nav'; gamepadState.navButtonIndex = gamepadState.hudButtonIndex > 1 ? 1 : 0; gamepadState.lastButtonTime = now; }
      else if (axisX < -0.4 && gamepadState.hudButtonIndex > 0) { gamepadState.hudButtonIndex--; gamepadState.lastButtonTime = now; }
      else if (axisX > 0.4 && gamepadState.hudButtonIndex < 2) { gamepadState.hudButtonIndex++; gamepadState.lastButtonTime = now; }

    } else if (gamepadState.focusZone === 'bottom') {
      if (axisY < -0.4) { gamepadState.focusZone = 'plots'; gamepadState.selectedPlotIndex = gamepadState.bottomButtonIndex === 0 ? 6 : 8; gamepadState.lastButtonTime = now; }
      else if (axisX < -0.4 && gamepadState.bottomButtonIndex > 0) { gamepadState.bottomButtonIndex--; gamepadState.lastButtonTime = now; }
      else if (axisX > 0.4 && gamepadState.bottomButtonIndex < 1) { gamepadState.bottomButtonIndex++; gamepadState.lastButtonTime = now; }
    }
  }
