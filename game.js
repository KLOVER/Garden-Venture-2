...
// 🌟 BULLETPROOF SERVER TIME SYNC VARS 🌟
...
function calculateProduceEarnings(base, rKg, minKg, isVine = false) {
  ...
  let m = 1 + Math.pow(weightRatio, 0.55) * 0.15;
  if (isVine) {
    m *= 0.3;
  }
...
function initFirebasePresence() {
  ...
  // Explicit sync grab for strict consoles like Xbox edge
  ...
  timeRef.set(firebase.database.ServerValue.TIMESTAMP).then(() => {
    timeRef.once('value').then(snap => {
        ...
        if(st) {
            ...
            timeSynced = true;
            updateGlobalCycle(); // Force exact global time immediately
        }
    });
  });
...
function secondTick() {
  if (timeSynced) {
    updateGlobalCycle();
    let sLeft = Math.floor((globalShopExpiresAt - getServerTime()) / 1000);
    if (sLeft <= 0) {
      sLeft = 0;
      if (isOnline) {
        globalShopExpiresAt = getServerTime() + 180000;
    ...
    // 🌟 FALLBACK: If Firebase hasn't given the exact sync yet, smoothly decrement local so we don't snap/jump 🌟
    ...
    shopRefillTimerEl.textContent = formatTime(Math.max(0, sLeft));
  }
...
function saveGame() {
  localStorage.setItem('gardenVenture2Save', JSON.stringify({
    ...
    cycleTimeLeft: gameState.cycleTimeLeft, // Store visually until sync happens
