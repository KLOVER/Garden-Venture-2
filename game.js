...
// 🌟 BUFFED STOCK ODDS MAP GENERATOR 🌟
function generateShopStockMap() {
  const stockMap = {};
  SEED_CATALOG.forEach(s => {
    let c = 1.0;
    if (s.rarity === 'uncommon') c = 0.75;
    else if (s.rarity === 'rare') c = 0.70;
    else if (s.rarity === 'legendary') c = 0.30; // 30% chance for Legendaries!

    if (s.id === 'sunflower') c = 0.75;
    else if (s.id === 'nectarroot') c = 0.50;
    else if (s.id === 'strawberry') c = 0.15;
    else if (s.id === 'cosmic_rose') c = 0.08;
    else if (s.id === 'singularity') c = 0.05; // 5% chance!
    else if (s.id === 'celestial_moon') c = 0.02; // 2% chance!

    stockMap[s.id] = (s.rarity === 'common' || Math.random() < c) ? s.maxStock : 0;
  });
  return stockMap;
}

function refreshShopStocks(silent = false) {
  const newStock = generateShopStockMap();
  SEED_CATALOG.forEach(s => {
    if (newStock[s.id] !== undefined) {
      s.currentStock = newStock[s.id];
    }
  });
  if (!silent) {
...
// 🌟 FIXED SHOP RESET: Never wipes node, creates new 3m timer + stock map 🌟
function checkAndResetGlobalShop() {
  const now = getServerTime();
  if (!globalShopExpiresAt || globalShopExpiresAt <= now) {
    globalShopExpiresAt = now + 180000;
    const newStock = generateShopStockMap();
    ...
    if (isOnline) {
      db.ref('globalShop').set({
        expiresAt: globalShopExpiresAt,
        stock: newStock
      });
    }
  }
...
function initFirebasePresence() {
  ...
  const timeRef = db.ref('serverTimeSync/' + myPlayerId);
  timeRef.set(firebase.database.ServerValue.TIMESTAMP).then(() => {
    timeRef.once('value').then(snap => {
        const st = snap.val();
        if(st) {
            serverTimeOffset = st - Date.now();
            timeSynced = true;
            updateGlobalCycle();
        }
    });
...
function secondTick() {
  ...
  if (timeSynced) {
    updateGlobalCycle();
    let sLeft = Math.max(0, Math.floor((globalShopExpiresAt - now) / 1000));
    if (sLeft <= 0) {
      checkAndResetGlobalShop();
      sLeft = Math.max(0, Math.floor((globalShopExpiresAt - now) / 1000));
    }
    ...
    let sLeft = Math.max(0, Math.floor((globalShopExpiresAt - Date.now()) / 1000));
    if (sLeft <= 0) {
      checkAndResetGlobalShop();
      sLeft = Math.max(0, Math.floor((globalShopExpiresAt - Date.now()) / 1000));
    }
    shopRefillTimerEl.textContent = formatTime(sLeft);
  }
...
function initGlobalShop() {
  ...
  db.ref('globalShop').on('value', snap => {
    const data = snap.val();
    if (data && data.stock && data.expiresAt && data.expiresAt > getServerTime()) {
      globalShopExpiresAt = data.expiresAt;
      SEED_CATALOG.forEach(s => {
        if (data.stock[s.id] !== undefined) {
          s.currentStock = data.stock[s.id];
        }
      });
      if (!shopModal.classList.contains('hidden')) {
        renderShopItems();
