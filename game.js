...
btnSellBargainModal.addEventListener('click', () => {
  ...
  npcDialogueText.textContent = `Greetings! I can appraise your Seed Bag. My fee is $${currentBargainFee.toLocaleString()}.`;
  npcBargainFee.textContent = `Fee: $${currentBargainFee.toLocaleString()}`;
  npcStandardValue.textContent = `$${bv.toLocaleString()}`;
...
// ==========================================
// MISSING SYSTEM & NETWORK HELPER FUNCTIONS
// ==========================================
...
function renderTradeSlots(myItems, partnerItems) {
  ...
  (myItems || []).forEach((item, index) => {
    ...
    slot.className = 'trade-slot-item';
    slot.style = "background:#fff; border:2px solid #8d6e63; border-radius:8px; padding:6px; display:flex; align-items:center; justify-content:space-between; margin-bottom:4px;";
    slot.innerHTML = `<span>${item.icon || '🌱'} ${item.name}</span>`;
    if (!amIReady) {
      const rmBtn = document.createElement('button');
      rmBtn.textContent = '❌';
      rmBtn.style = "background:none; border:none; cursor:pointer;";
      rmBtn.onclick = () => {
  ...
  (partnerItems || []).forEach(item => {
    const slot = document.createElement('div');
    slot.className = 'trade-slot-item';
    slot.style = "background:#fff; border:2px solid #8d6e63; border-radius:8px; padding:6px; margin-bottom:4px;";
    slot.innerHTML = `<span>${item.icon || '🌱'} ${item.name}</span>`;
    theirTradeSlots.appendChild(slot);
  });
}
