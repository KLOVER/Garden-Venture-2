...
function calculateProduceEarnings(basePrice, rolledKg, minKg, isVine = false) {
  ...
  if (isVine) multiplier *= 0.3; // Rebalance Vine Payouts heavily!
...
function openSkipModal(plotIndex, isFruit = false, fruitIndex = 0) {
  ...
  const cost = Math.ceil(remainingSecs * (crop.baseSellPrice * 0.8 + 5));
...
function handlePlotClick(plotIndex, event) {
  ...
  // Vine Trellis (Not yet established)
  if (plot.crop && plot.crop.isVine && !plot.vineEstablished) { 
    openSkipModal(plotIndex); 
    return; 
  }
  
  // Established Vine Menu
  if (plot.crop && plot.crop.isVine && plot.vineEstablished) { 
  ...
  if (plot.crop && plot.isReady) {
    ...
    // NERFED XP GAIN (Slows down leveling!)
    const xpReward = Math.ceil(harvestedCrop.baseGrowTime * 3);
    addXP(xpReward);
  ...
  (plot.vineFruits || []).forEach((fruit, idx) => {
    ...
    if (fruit.isReady) {
      actionBtn.addEventListener('click', () => {
        // Divide XP by the max fruits so vine plants don't level you up 3x faster than normal crops!
        const xpReward = Math.ceil((plot.crop.baseGrowTime * 3) / (plot.crop.maxFruits || 3)); 
        addXP(xpReward);
...
function saveGame() {
  localStorage.setItem('gardenVenture2Save', JSON.stringify({
    cash: gameState.cash, level: gameState.level, xp: gameState.xp, 
    currentField: gameState.currentField, unlockedFields: gameState.unlockedFields,
...
function loadGame() {
  ...
  if (savedData) {
    try {
      ...
      gameState.cash = data.cash || 25; gameState.level = data.level || 1; gameState.xp = data.xp || 0;
      gameState.currentField = data.currentField || 0; gameState.unlockedFields = data.unlockedFields || 1;
