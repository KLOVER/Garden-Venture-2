...
// ==========================================================================
// 🌟 PLANT & SEED CATALOGS (CALIBRATED V2.0 / V4 ECONOMY)
// ==========================================================================
...
const FENCE_SKINS_CATALOG = [
  { id: 'twig-tangle', name: 'Twig Tangle', rarity: 'common', cost: 1500, maxStock: 5, currentStock: 5 },
  { id: 'garden-rail', name: 'Garden Rail', rarity: 'common', cost: 3500, maxStock: 5, currentStock: 5 },
  { id: 'bamboo-braid', name: 'Bamboo Braid', rarity: 'uncommon', cost: 18000, maxStock: 2, currentStock: 2 },
  { id: 'mossbound', name: 'Mossbound', rarity: 'uncommon', cost: 45000, maxStock: 2, currentStock: 2 },
...
let isAdminAuthenticated = localStorage.getItem('gv2_admin_auth') === 'true';
...
// 🛒 CALCULATE CYCLE STOCKS (BALANCED WITH LUCK MULTIPLIERS)
function getGlobalShopStockForCycle(cycleId, luckMult = 1.0) {
  ...
  // Event Seeds (Holofern exact 0.01% on its own, scaled by luck)
  ...
  FENCE_SKINS_CATALOG.forEach(skin => {
    ...
    stockMap.fences[skin.id] = (skin.rarity === 'common' || rng() < adjustedChance) ? skin.maxStock : 0;
  });
...
function updateShopForCurrentCycle(silent = false) {
  ...
  if (gameState.lastShopCycle !== cycleId) {
    ...
    if (decorModal && !decorModal.classList.contains('hidden')) renderDecorShop();
...
const adminLuckSelect = el('admin-luck-select');
const adminBtnTriggerLuck = el('admin-btn-trigger-luck');
const adminBtnToggleWeather = el('admin-btn-toggle-weather');
...
// ==========================================================================
// 🎨 PRISMATIC GLOBS VISUALS (IN FOREST BACKGROUND)
// ==========================================================================
...
// ==========================================================================
// 🌧️ FUNCTIONAL HOLOGRAPHIC RAIN & MUTATION
// ==========================================================================
...
// ==========================================================================
// 🌧️ WEATHER SYNCHRONIZATION ENGINE & OVERRIDES (EXACT 30% NIGHT CHANCE)
// ==========================================================================

function updateGlobalCycle() {
  const CYCLE_DURATION = 600000; // 10-minute global cycle
  const HALF_CYCLE = 300000;     // 5-minute day, 5-minute night
  ...
  if (gameState.weatherOverride) {
    ...
    if (nowOffset < HALF_CYCLE) {
      ...
      const isRainRoll = (nightRng() < 0.30); // Exact 30% roll
...
// ==========================================================================
// 🔐 DIRECT-FIREBASE GLOBAL ADMIN SUITE & ONE-TIME UNLOCK
// ==========================================================================
...
if (btnSubmitAdmin) {
  btnSubmitAdmin.addEventListener('click', () => {
    if (adminPass1.value === "0313" && adminPass2.value === "789") {
      ...
      localStorage.setItem('gv2_admin_auth', 'true');
      populateAdminDropdowns();
      openModal(adminModal);
      closeModal(adminLoginModal);
      showToast("🛡️ Admin Authenticated (Saved)");
      adminPass1.value = "";
      adminPass2.value = "";
    } else {
...
// 🍀 Apply Restock Luck Trigger (Local & Global)
if (adminBtnTriggerLuck) {
  adminBtnTriggerLuck.addEventListener('click', () => {
    const luckMult = parseFloat(adminLuckSelect.value) || 1.0;
    if (adminTargetScope.value === 'global' && !isPlaytesterMode) {
      db.ref('globalRestockLuck').set({ multiplier: luckMult, ts: getServerTime() });
      showToast(`Sent Global Luck Command: ${luckMult}X`);
    } else {
      gameState.restockLuckMultiplier = luckMult;
      gameState.lastShopCycle = null;
      updateShopForCurrentCycle();
      showToast(`🍀 Local Restock Luck set to ${luckMult}X!`);
    }
  });
}

// 🌧️ Prismatic Weather Override Toggle (Local & Global)
if (adminBtnToggleWeather) {
  adminBtnToggleWeather.addEventListener('click', () => {
    const newState = !gameState.weatherOverride;
    if (adminTargetScope.value === 'global' && !isPlaytesterMode) {
      db.ref('globalWeatherOverride').set({ active: newState, ts: getServerTime() });
      showToast(`Sent Global Weather Override: ${newState ? 'ON' : 'OFF'}`);
    } else {
      gameState.weatherOverride = newState;
      updateGlobalCycle();
      showToast(`🌧️ Weather Override: ${newState ? 'PRISMATIC RAIN ON' : 'AUTO CYCLE RESTORED'}`);
    }
  });
}
...
if (adminBtnRestock) {
  adminBtnRestock.addEventListener('click', () => {
    ...
    if (adminTargetScope.value === 'global' && !isPlaytesterMode) {
      db.ref('adminCommands').push({ type: 'restock', seedId: id, amount: q, ts: getServerTime() });
      showToast(`Sent Global Command: Restock ${id} to ${q}`);
    } else {
...
if (adminBtnSkipGrow) {
  adminBtnSkipGrow.addEventListener('click', () => {
    if (adminTargetScope.value === 'global' && !isPlaytesterMode) {
      ...
      showToast(`Sent Global Fast-Forward Command`);
    } else {
      let skipped = 0;
...
// ==========================================================================
// 🤠 HARVEST MARKET, BARGAIN & 12H DAILY DEAL ENGINE
// ==========================================================================
...
btnSellBargainModal.addEventListener('click', () => {
  ...
  gameState.produceInventory.forEach(i => {
    bv += i.value;
    if (i.seedId === 'singularity' || i.seedId === 'celestial_moon' || i.seedId === 'strawberry' || i.seedId === 'cosmic_rose' || i.isHolo || i.isOG || i.seedId === 'holofern') hR++;
  });
  ...
  dailyDealWrapper.classList.add('hidden');
...
btnStartBargain.addEventListener('click', () => {
  ...
  dailyDealWrapper.classList.remove('hidden');
...
btnDailyDeal.addEventListener('click', () => {
  const TWELVE_HOURS = 12 * 60 * 60 * 1000;
  const now = Date.now();
  const elapsed = now - (gameState.lastDailyDealTime || 0);

  if (elapsed >= TWELVE_HOURS && currentBargainBase > 0) {
    isDailyDealActive = true;
    gameState.lastDailyDealTime = now;
...
function initFirebasePresence() {
  ...
  // Global Weather Override Listener
  ...
  // Global Admin Commands Listener (Ignore existing backlog on fresh load)
  ...
  db.ref('adminCommands').limitToLast(20).on('child_added', snap => {
    if (!initialBacklogLoaded) return; // Prevent replaying old commands on refresh
    const cmd = snap.val();
    if (!cmd) return;
...
function loadGame() {
  ...
  if (sd) {
    try {
      ...
      gameState.weatherOverride = d.weatherOverride || false;
